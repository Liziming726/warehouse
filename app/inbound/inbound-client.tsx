...
import { createInbound } from '@/app/inbound/actions';
import type {
  InventoryAccess,
  MovementRow,
  ProductOption,
...
function toFormData(values: InboundFormValues) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      startTransition(async () => {
        try {
          await createInbound(toFormData(payload));
          messageApi.success('Inbound record created.');
          form.resetFields();
          form.setFieldsValue({
          ...
          messageApi.error(
            error instanceof Error ? error.message : 'Failed to create inbound record.'
          );
        }
      });
    } catch {
    ...
    <Space direction="vertical" size="large" style={{ width: '100%' }}>
      ...
      {loadErrorMessage ? (
        <Alert
          type="error"
          showIcon
          message="Failed to load inbound page data"
      ...
      <Card title="Create Inbound Record">
        <Form
          form={form}
          layout="vertical"
          initialValues={{
